    ...
    try:
        with app.store.transaction() as connection:
            ...
            connection.executemany(
                "INSERT INTO frame_probe VALUES(?)", [(1,), (2,), (3,)],
            )
        app.store.checkpoint("TRUNCATE")

        def failed_frame(*_args, **_kwargs):
            reader = app.store.read()
            cursor = reader.execute("SELECT value FROM frame_probe")
            assert cursor.fetchone()[0] == 1
            # The Future's exception retains this frame and its open cursor.
            reader.execute("SELECT value FROM missing_frame_table")

        monkeypatch.setattr(app, "frame", failed_frame)
        with pytest.raises(sqlite3.OperationalError) as failure:
            deadline = time.monotonic() + 2
            while time.monotonic() < deadline:
                app.poll_frame({}, -1, 0)
                time.sleep(0.01)
            pytest.fail("frame failure was not delivered")
        assert failure.value.sqlite_errorcode == sqlite3.SQLITE_ERROR
        with app.store.transaction() as connection:
            connection.execute("INSERT INTO frame_probe VALUES(4)")
        checkpoint = app.store.checkpoint("TRUNCATE")
        assert checkpoint["busy"] == 0
        assert checkpoint["wal_bytes"] == 0
        assert [row[0] for row in app.store.read().execute(
            "SELECT value FROM frame_probe ORDER BY value"
        )] == [1, 2, 3, 4]
    finally:
        app.close()
...
def test_pool_refresh_finishes_when_shared_aggregate_waits_for_reader_drain(
        tmp_path, monkeypatch):
    ...
    def controlled_snapshot(*args, **kwargs):
        ...
        if role == "aggregate-producer" and not nested:
            aggregate_entered.set()
            assert release_aggregate.wait(5)
    ...
    try:
        ...
        aggregate.start()
        assert aggregate_entered.wait(2)

        # The pool owns a snapshot; the aggregate owns a shared Future but
        # has not yet entered SQLite. Closing admission creates a wait cycle
        # if the pool waits for that Future instead of using its own snapshot.
        draining = app.store.checkpoint("RESTART", drain_readers=True)
        assert draining["reader_drain_pending"] == 1
        release_aggregate.set()
        release_pool.set()
        assert pool_done.wait(2), "pool refresh pinned the checkpoint drain"
        assert failures == {}
        row = next(row for row in results["pools"]["rows"] if row["id"] == V3)
        assert row["swaps"] == 1
        assert app.store.checkpoint("RESTART", drain_readers=True)["busy"] == 0
...
def test_cold_v4_activity_resolves_only_a_complete_verified_pool_key(
    ...
    def rpc_batch(calls, **_kwargs):
        return [rpc(method, params) for method, params in calls]

    for client in (indexer._clients["pool"], indexer._clients["receipt"]):
        monkeypatch.setattr(client, "call", rpc)
        monkeypatch.setattr(client, "batch", rpc_batch)
        monkeypatch.setattr(client, "batch_results", rpc_batch)
    try:
        cursor = indexer.feed_updates()
