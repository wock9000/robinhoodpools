...
The runtime prepares unsigned direct-pool remove and collect transaction data only when explicitly enabled. It rejects add-liquidity execution, including preparation from stale add or approval quotes; see SECURITY.md for the retired add router advisory. It does not store private keys, sign transactions, or broadcast transactions.
